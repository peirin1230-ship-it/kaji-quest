...
10. **実績** に称号・バッジ・領域ごとのレベル・「あと少し」のバッジが出る。「トロフィールームを開く」で全バッジの一覧（未獲得は進み具合、シークレットは ???）。バッジは記録から毎回計算するので、取ったものは記録が残るかぎり残る
11. **掃除の教科書** は、洗浄の原理・汚れの見分け方・洗剤のラベル・安全・除菌・洗濯・素材別の可否をやさしい言葉で書いた 9 章。章をタップで開く。各章の最後に、法令や公的機関を根拠にした元の資料（`knowledge/reference/`）へのリンクがある
