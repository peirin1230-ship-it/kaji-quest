...
3. 今日のタスクが朝・昼・夜に分かれて並ぶ。やったら **完了** を 1 タップ。所要時間は見込みが自動で入る。変えたいときだけ「詳細」（1〜3 分の小さな単位や時間帯も選べる）。同じタスクを何回にも小分けして記録できる（完了のあと「追加」）
...
8. 上の日付を ‹ › で動かすか日付をタップすると、過去の日の記録を見たり、その日に記録し忘れた分を後から入れたりできる。「今週」の棒グラフの日をタップしても移動する
9. 一番上の検索欄で、タスク・掃除メニュー・コツ・過去の記録をまとめて探せる。結果からそのまま完了や実践を記録できる
10. **掃除の教科書** は、洗浄の原理・汚れの見分け方・洗剤のラベル・安全・除菌・洗濯・素材別の可否をやさしい言葉で書いた 9 章。章をタップで開く。各章の最後に、法令や公的機関を根拠にした元の資料（`knowledge/reference/`）へのリンクがある
...
| したいこと | 場所 |
|---|---|
| タスクを足す・減らす・見込みや負荷 W を変える・朝昼夜の欄を変える（`slots:`） | `routines/daily.yml`（push すると自動で反映） |
